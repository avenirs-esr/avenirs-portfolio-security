package fr.avenirsesr.portfolio.security.accesscontrol.infrastructure.adapter.repository;

import fr.avenirsesr.portfolio.security.accesscontrol.infrastructure.adapter.model.RBACRoleEntity;
import java.util.Optional;
import java.util.UUID;
import org.springframework.data.jpa.repository.JpaRepository;

/**
 *
 *
 * <h1>RBACRoleRepository</h1>
 *
 * <p><b>Description:</b> Repository for RBAC Roles.
 *
 * <h2>Version:</h2>
 *
 * 1.0.0
 *
 * <h2>Author:</h2>
 *
 * Arnaud Deman
 *
 * <h2>Since:</h2>
 *
 * 15/10/2024
 */
public interface RBACRoleJpaRepository extends JpaRepository<RBACRoleEntity, UUID> {
  /**
   * Find a role by its name.
   *
   * @param name The name of the role to retrieve.
   * @return An Optional of RBACRole.
   */
  Optional<RBACRoleEntity> findByName(String name);
}
