...
NUM_ROLES_AND_ACTIONS = 1

MAX_PRINCIPAL_ASSIGNMENTS = 100
MAX_SCOPE_RESOURCES = 1
...
class DataGenerator:
    ...
    def generate_resources(self, resource_type_ids):
        return [{
            "id": str(uuid.uuid4()),
            "selector": "resource_0",
            "id_resource_type": resource_type_ids[0],
        }]


    # For each principal the role is assigned with a probability of 0.5
    def generate_assignments(self, role_ids, principal_ids):
        ...
        for id_principal in principal_ids:
            if random.choice([True, False]):  # 1 chance sur 2 d'assigner le rôle
                id_scope = str(uuid.uuid4())
                scope = {"id": id_scope, "name": "scope_0"}
                scopes.append(scope)

                id_context = str(uuid.uuid4())
                context = {"id": id_context}
    ...
    def generate_context_structures(self, context_ids, structure_ids):
        ...
        for id_context in context_ids:
            ...
            nb_structures = random.randint(1, MAX_CONTEXT_STRUCTURES)

            for _ in range(min(nb_structures, MAX_NUM_STRUCTURES)):
    ...
    def generate_action_routes(self, action_ids):
        return [{
            "id": str(uuid.uuid4()),
            "id_action": action_ids[0],
            "uri": "/ac_integration",
            "method": "GET",
            "description": "Route to test access control integration",
        }]
...
def generate_fixtures(multiplicator=1):
    ...
    output_directory = f"target/fixtures/ac-integration/m{multiplicator}"
    ...
    logger.info(f"Number of resources: 1")
    ...
    logger.info("Step : resources (%s)", 1)
