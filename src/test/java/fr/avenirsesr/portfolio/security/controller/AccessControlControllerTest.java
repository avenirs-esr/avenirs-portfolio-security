 ...
 *
 * <p>Description: general test case, bad header, missing token, etc. See specific tests are located
...
class AccessControlControllerTest {
  ...
  @InjectMocks private AccessControlController accessControlController;
  ...
  void grantAccessSuccess() {
    ...
    AccessControlGrantResponse grantResponse =
        new AccessControlGrantResponse().setLogin("user123").setGranted(true);

    when(accessControlService.grantAccess(any(AccessControlGrantRequest.class)))
        .thenReturn(grantResponse);

    ResponseEntity<AccessControlGrantResponse> response =
        accessControlController.grantAccess(grantRequest);
  ...
  void revokeAccessSuccess() {
    ...
    AccessControlRevokeResponse revokeResponse =
        new AccessControlRevokeResponse().setLogin("user123").setRevoked(true);

    when(accessControlService.revokeAccess(any(AccessControlRevokeRequest.class)))
        .thenReturn(revokeResponse);

    ResponseEntity<AccessControlRevokeResponse> response =
        accessControlController.revokeAccess(revokeRequest);
  ...
  void testIsAuthorizedWithoutHeader() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.get(accessControlEndPoint)
