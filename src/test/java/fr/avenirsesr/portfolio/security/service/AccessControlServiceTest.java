 ...
 *
 * <p><b>Description:</b> based on test case 1: user gribonvald is pair for a resource of type MES.
 * * * For more details * <a
 * href="https://avenirs-esr.github.io/dev-doc/arch-soft-specif-security-rbac-test-case2/">*
 * https://avenirs-esr.github.io/dev-doc/arch-soft-specif-security-rbac-test-case2/ * </a> *
 *
 * <h2>Version:</h2>
 *
 * 1.0.0
...
class AccessControlServiceTest {
  ...
  @SpyBean @Autowired private AccessControlService accessControlService;
  ...
  void isAuthorized() {
    ...
    doAnswer(
        ...
        .createExecutionContext(any(Principal.class));

    assertTrue(
        accessControlService.isAuthorized(
            principal.getLogin(), display.getId(), authorizedResource.getId()),
        "Display access on authorized resource.");
    assertFalse(
        accessControlService.isAuthorized(
            principal.getLogin(), display.getId(), unauthorizedResource.getId()),
        "Display access on unauthorized resource.");
    assertFalse(
        accessControlService.isAuthorized(
            principal.getLogin(), edit.getId(), authorizedResource.getId()),
        "Edit access on authorized resource.");
    assertFalse(
        accessControlService.isAuthorized(
            principal.getLogin(), edit.getId(), unauthorizedResource.getId()),
        "Edit access on unauthorized resource.");
    assertTrue(
        accessControlService.isAuthorized(
            principal.getLogin(), feedback.getId(), authorizedResource.getId()),
        "Feedback access on authorized resource.");
    assertFalse(
        accessControlService.isAuthorized(
            principal.getLogin(), feedback.getId(), unauthorizedResource.getId()),
        "Feedback access on unauthorized resource.");
  }
  ...
  void grantAccess() {
    ...
    AccessControlGrantRequest grantRequest =
        new AccessControlGrantRequest()
            .setLogin(userLogin)
    ...
    assertThat(assignment.getScope().getResources().stream().map(RBACResource::getId))
        .containsExactlyInAnyOrder(grantResourceIds);
    assertEquals(
        validityStart, assignment.getContext().getValidityStart(), "Assignment validity start");
    assertEquals(validityEnd, assignment.getContext().getValidityEnd(), "Assignment validity end");
    assertThat(assignment.getContext().getStructures().stream().map(Structure::getId))
        .containsExactlyInAnyOrder(grantStructureIds);
  }
  ...
  void grantAccessWithInvalidPrincipal() {
    ...
    EntityNotFoundException exception =
        assertThrows(
            EntityNotFoundException.class, () -> accessControlService.grantAccess(grantRequest));
  ...
  void grantAccessWithInvalidRole() {
    ...
    assertEquals(
        "Role not found, ID: 00000000-0000-0000-0000-000000000123",
  ...
  void grantAccessWithInvalidResource() {
    ...
    Exception exception =
        assertThrows(
            EntityNotFoundException.class, () -> accessControlService.grantAccess(grantRequest));
    assertEquals(
        "Missing resources, IDs : [00000000-0000-0000-0000-000000000100]",
        exception.getMessage(),
        "Exception message for missing resources");
  }
  ...
  void grantAccessWithInvalidStructure() {
    ...
    Exception exception =
        assertThrows(
            EntityNotFoundException.class, () -> accessControlService.grantAccess(grantRequest));
    assertEquals(
        "Missing structures, IDs : [00000000-0000-0000-0000-000000000100]",
        exception.getMessage(),
        "Exception message for missing resources");
  }
  ...
  void grantAccessWithInvalidValidityStart() {
    ...
    Exception exception =
        assertThrows(RuntimeException.class, () -> accessControlService.grantAccess(grantRequest));
    assertTrue(
        exception.getMessage().contains("Invalid validity start date format"),
        "Exception message for invalid start date format");
  }
  ...
  void grantAccessWithInvalidValidityEnd() {
    ...
    Exception exception =
        assertThrows(RuntimeException.class, () -> accessControlService.grantAccess(grantRequest));
    assertTrue(
        exception.getMessage().contains("Invalid validity end date format"),
        "Exception message for invalid end date format");
  }
  ...
  void revokeAccess() {
    ...
    AccessControlRevokeResponse response =
        accessControlService.revokeAccess(
            new AccessControlRevokeRequest()
                .setLogin(userLogin)
