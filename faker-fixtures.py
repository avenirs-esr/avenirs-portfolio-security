...
class DataGenerator:
    ...
    def generate_context_structures(self, context_ids, structure_ids):
        ...
        for id_context in context_ids:
            num_structures = random.randint(1, MAX_CONTEXT_STRUCTURES)
...
def generate_fixtures():
    ...
    output_directory = "target/fixtures"
    changelog_file = "fixtures-changelog.yaml"

    logger.info("---- Starting data generation ----")

    logger.info("Step : principals (%s)", NUM_PRINCIPALS)
    ...
    logger.info("Step : resources (%s)", NUM_RESOURCES)
