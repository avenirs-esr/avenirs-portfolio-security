...
class RBACActionServiceTest {
  ...
  private static final UUID SHARE_READ_RESOURCE_ID =
  ...
  private static final UUID DO_FEEDBACK_ID =
  ...
  private static final String[] EXPECTED_ACTION_NAMES = {
  ...
  @Autowired private RBACActionService actionService;
  ...
  void testGetActionByNameShareRead() {
    ...
    assertThat(actual.getPermissions().stream().map(p -> p.getName()))
        .contains(SHARE_READ_RESOURCE_PERMS);
  }
