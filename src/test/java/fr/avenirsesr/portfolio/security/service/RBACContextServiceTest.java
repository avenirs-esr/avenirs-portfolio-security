...
class RBACContextServiceTest {
  ...
  @Autowired private RBACContextService contextService;

  @PostConstruct
  public void init() {
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy/MM/dd");
    this.contextValidityStart =
        LocalDate.parse(contextValidityStartString, formatter).atStartOfDay();
    this.contextValidityEnd = LocalDate.parse(contextValidityEndString, formatter).atStartOfDay();
    this.newContextValidityStart =
        LocalDate.parse(newContextValidityStartString, formatter).atStartOfDay();
    this.newContextValidityEnd =
        LocalDate.parse(newContextValidityEndString, formatter).atStartOfDay();
  }
  ...
  @Test
  void createContext() {
    RBACContext newContext =
        new RBACContext()
            .setValidityStart(newContextValidityStart)
            .setValidityEnd(newContextValidityEnd)
            .setStructures(
                Arrays.stream(newContextStructureNames)
                    .map(name -> new Structure().setName(name).setDescription(""))
    ...
    RBACContext fetchedContext =
        contextService
            .getContextById(savedContext.getId())
            .orElseThrow(
                () -> new AssertionError("Context not found with ID: " + savedContext.getId()));
  ...
  void updateContext() {
        ...
        new RBACContext()
            ...
            .setStructures(
                Collections.singleton(
                    new Structure().setName(newContextStructureNames[0]).setDescription("")));
    ...
    RBACContext fetchedContext =
        contextService
            .getContextById(newContext.getId())
            .orElseThrow(
                () -> new AssertionError("Context not found with ID: " + newContext.getId()));

    assertEquals(
        newContextValidityStart.plusDays(1),
        fetchedContext.getValidityStart(),
        "Updated context validity start");
    assertEquals(
        newContextValidityEnd, fetchedContext.getValidityEnd(), "Updated context validity end");
    assertEquals(1, fetchedContext.getStructures().size());
    assertEquals(
        newContextStructureNames[0],
        fetchedContext.getStructures().toArray(new Structure[0])[0].getName(),
        "Updated context, structure name");
  }
