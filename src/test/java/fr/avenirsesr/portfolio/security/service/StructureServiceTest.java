...
class StructureServiceTest {
  ...
  void getStructureById() {
    ...
    Optional<Structure> response =
        structureService.getStructureById(UUID.fromString("00000000-0000-0000-0000-000000000100"));
  ...
  void getStructureByName() {
    ...
    Structure structure =
        structureService
            ...
            .orElseThrow(
                () -> new AssertionError("Structure not found with Name: " + structureName));
  ...
  void getAllStructures() {
    ...
    assertThat(actual.stream().map(Structure::getName))
        .containsExactlyInAnyOrder(allStructureNames);
  }

  @Test
  void getAllStructuresBySpecification() {
    List<Structure> actual =
        structureService.getAllStructuresBySpecification(
            StructureSpecificationHelper.filterByIds(filteredStructureIds));
    assertThat(actual).hasSize(filteredStructureIds.length);
    assertThat(actual.stream().map(Structure::getId))
        .containsExactlyInAnyOrder(filteredStructureIds);
  }

  @Test
  void createStructure() {
    Structure newStructure =
        new Structure().setName(newStructureName).setDescription(newStructureDescription);
    ...
    Structure fetchedStructure =
        structureService
            .getStructureById(savedStructure.getId())
            .orElseThrow(
                () -> new AssertionError("Structure not found with ID: " + savedStructure.getId()));

    assertEquals(newStructureName, fetchedStructure.getName(), "new Structure name");
    assertEquals(
        newStructureDescription, fetchedStructure.getDescription(), "new Structure description");
  }
  ...
  void deleteStructure() {
    ...
    Optional<Structure> fetchedStructure =
        structureService.getStructureById(savedStructure.getId());
