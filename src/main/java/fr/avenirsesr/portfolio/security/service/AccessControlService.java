 ...
 *
 * <p><b>Description:</b> AccessControlService is used to check if a resource can be accessed by a
 * principal. It is an RBAC: Role Based Access Control.
 *
 * <h2>Version:</h2>
 *
 * 1.0
...
public class AccessControlService {
  ...
  /** Action service */
  @Autowired RBACActionService actionService;

  /** Assignment service. */
  @Autowired RBACAssignmentService assignmentService;

  @Autowired RBACResourceService resourceService;

  @Autowired PrincipalService principalService;

  @Autowired RBACRoleService roleService;
  ...
  public AccessControlGrantResponse grantAccess(AccessControlGrantRequest grantRequest) {
    ...
    RBACRole role =
        roleService
            .getRoleById(grantRequest.getRoleId())
            .orElseThrow(
                () ->
                    new EntityNotFoundException("Role not found, ID: " + grantRequest.getRoleId()));

    Principal principal =
        principalService
            .getPrincipalByLogin(grantRequest.getLogin())
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        "Principal not found, UID: " + grantRequest.getLogin()));
    ...
    if (!ObjectUtils.isEmpty(grantRequest.getResourceIds())) {
      ...
      List<UUID> foundIds = resources.stream().map(RBACResource::getId).toList();

      List<UUID> missingIds =
          Arrays.stream(grantRequest.getResourceIds())
    ...
    if (StringUtils.hasLength(grantRequest.getValidityStart())) {
      try {
        applicationContext.setValidityStart(
            LocalDate.parse(grantRequest.getValidityStart(), formatter).atStartOfDay());
      } catch (DateTimeParseException e) {
        throw new RuntimeException(
            "Invalid validity start date format ("
                + dateFormat
                + "): "
                + grantRequest.getValidityStart());
      }
    }

    if (StringUtils.hasLength(grantRequest.getValidityEnd())) {
      try {
        applicationContext.setValidityEnd(
            LocalDate.parse(grantRequest.getValidityEnd(), formatter).atStartOfDay());
      } catch (DateTimeParseException e) {
        throw new RuntimeException(
            "Invalid validity end date format ("
                + dateFormat
                + "): "
                + grantRequest.getValidityEnd());
      }
    }
    ...
    if (!ObjectUtils.isEmpty(grantRequest.getStructureIds())) {
      List<Structure> structures =
          structureService.getAllStructuresBySpecification(
              StructureSpecificationHelper.filterByIds(grantRequest.getStructureIds()));

      List<UUID> foundIds = structures.stream().map(Structure::getId).toList();

      List<UUID> missingIds =
          Arrays.stream(grantRequest.getStructureIds())
              .filter(id -> !foundIds.contains(id))
              .collect(Collectors.toList());
  ...
  public AccessControlRevokeResponse revokeAccess(AccessControlRevokeRequest revokeRequest) {
    ...
    assignmentService.deleteAssignment(revokeRequest.getAssignmentId());
    return new AccessControlRevokeResponse()
        .setLogin(revokeRequest.getLogin())
        .setAssignmentId(revokeRequest.getAssignmentId())
        .setRevoked(true);
  }

  /**
   * Checks that a principal has access to a resource to perform an action, without application
   * context.
   *
  ...
  public boolean isAuthorized(String login, UUID actionId, UUID resourceId) {
    ...
    List<RBACAssignment> principalAssignments =
        this.assignmentService.getAllAssignmentsBySpecification(
            RBACAssignmentSpecificationHelper.filterByPrincipalContextAndResources(
                login, executionContext, resourceId));
   ...
   */
  public RBACContext createExecutionContext(
      Principal principal) { // Need to be public to be mocked in tests.
