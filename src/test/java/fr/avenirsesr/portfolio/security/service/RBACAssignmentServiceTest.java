...
class RBACAssignmentServiceTest {
  ...
  void createAssignment() {
    ...
    RBACContext context =
        new RBACContext().setId(UUID.fromString("00000000-0000-0000-0000-000000000100"));
    ...
    assertTrue(
        fetchedAssignment.getContext().getStructures().isEmpty(), "Context empty structures");
  ...
  void getAllAssignmentsBySpecificationForPrincipal() {
    ...
    assignmentService.createAssignment(
        new RBACAssignment()
            ...
            .setRole(owner)
            .setPrincipal(principal1)
            .setScope(new RBACScope().setResources(Collections.singletonList(resource1))));
    ...
    List<RBACAssignment> fetchedAssignments =
        assignmentService.getAllAssignmentsBySpecification(
            RBACAssignmentSpecificationHelper.filterByPrincipal(principalLogin1));
    ...
    fetchedAssignments =
        assignmentService.getAllAssignmentsBySpecification(
            RBACAssignmentSpecificationHelper.filterByPrincipal(principalLogin2));
  ...
  void getAllAssignmentsBySpecificationPrincipalAndResources() {
    ...
    assignmentService.createAssignment(
        new RBACAssignment()
            ...
            .setRole(owner)
            .setPrincipal(principal1)
            .setScope(new RBACScope().setResources(Arrays.asList(resource1, resource2))));
    ...
    assignmentService.createAssignment(
        new RBACAssignment()
            ...
            .setPrincipal(principal1)
            .setScope(
                new RBACScope().setResources(Arrays.asList(resource1, resource2, resource3))));
    ...
    List<RBACAssignment> fetchedAssignments =
        assignmentService.getAllAssignmentsBySpecification(
            RBACAssignmentSpecificationHelper.filterByPrincipalAndResources(
                principalLogin1, resourceId1));
    assertEquals(
        3,
        fetchedAssignments.size(),
        "Number of fetched Assignments for principal 1 and resource 1");
    assertThat(fetchedAssignments)
        .as("Checks that all assignments are for principal1")
        .extracting(assignment -> assignment.getPrincipal().getLogin())
        .allMatch(login -> login.equals(principalLogin1));
    ...
    fetchedAssignments =
        assignmentService.getAllAssignmentsBySpecification(
            RBACAssignmentSpecificationHelper.filterByPrincipalAndResources(
                principalLogin1, resourceId1, resourceId2));
    assertEquals(
        4,
        fetchedAssignments.size(),
        "Number of fetched Assignments for principal 1 and resource 1 and 2");
    assertThat(fetchedAssignments)
        .as("Checks that all assignments are for principal1")
        .extracting(assignment -> assignment.getPrincipal().getLogin())
        .allMatch(login -> login.equals(principalLogin1));
