...
class RBACRoleServiceTest {
  ...
  @Autowired private RBACRoleService roleService;
  ...
  void getRoleById() {
    ...
    Optional<RBACRole> response =
        roleService.getRoleById(UUID.fromString("00000000-0000-0000-0000-000000000100"));
  ...
  void createRole() {
    RBACRole newRole =
        new RBACRole()
            ...
            .setPermissions(
                Arrays.stream(newRolePermissionNames)
                    .map(name -> new RBACPermission().setName(name).setDescription(""))
                    .collect(Collectors.toList()));

    RBACRole savedRole = roleService.createRole(newRole);
    assertNotNull(savedRole);

    RBACRole fetchedRole =
        roleService
            .getRoleById(savedRole.getId())
