...
import fr.avenirsesr.portfolio.security.repository.RBACScopeSpecificationHelper;
...
class RBACScopeServiceTest {
  ...
  @Autowired private RBACScopeService scopeService;
  ...
  void getScopeById() {
    ...
    Optional<RBACScope> response =
        scopeService.getScopeById(UUID.fromString("00000000-0000-0000-0000-000000000100"));
  ...
  void getAllScopesBySpecification() {
    ...
    List<RBACScope> actual =
        scopeService.getAllScopesBySpecification(
            RBACScopeSpecificationHelper.filterByResources(resourcesFilter));
  ...
  void createScope() {
    RBACScope newScope =
        new RBACScope()
            ...
            .setResources(
                IntStream.range(0, newScopeResourcesNumber)
                    .mapToObj(
                        i ->
                            new RBACResource()
                                .setSelector("*")
                                .setResourceType(
                                    new RBACResourceType().setId(newScopeResourcesTypeId)))
                    .collect(Collectors.toList()));
    RBACScope savedScope = scopeService.createScope(newScope);
    assertNotNull(savedScope);

    RBACScope fetchedScope =
        scopeService
            .getScopeById(savedScope.getId())
            .orElseThrow(
                () -> new AssertionError("Scope not found with ID: " + savedScope.getId()));
