...
/** Assignment specification for ActionRoute model. Used to make queries based on predicates. */
@Slf4j
public abstract class RBACActionRouteSpecificationHelper {

  /**
   * Specification to generate predicate to select the action associated to an uri and a method
   * (http).
   *
  ...
  public static Specification<RBACActionRoute> filterByURIAndMethod(String uri, String method) {
    return (Root<RBACActionRoute> root,
        CriteriaQuery<?> query,
      ...
      return criteriaBuilder.and(
          ...
          criteriaBuilder.equal(
              criteriaBuilder.lower(root.get(RBACActionRoute_.method)), lcMethod));
    };
  }
}
