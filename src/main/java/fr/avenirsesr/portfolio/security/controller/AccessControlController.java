 ...
 *
 * <p><b>Description:</b> AccessControlController is used to:
 *
 * <ul>
 *   <li>Create assignment to grant privileges
 *   <li>Update assignment to modify granted privileges
 *   <li>Delete assignment to revoke privileges
 *   <li>Check assignment to determine if an action is authorized for a principal
 * </ul>
 *
 * <h2>Version:</h2>
 *
...
public class AccessControlController {
  ...
  @Autowired private RBACActionRouteService actionRouteService;
  ...
  /**
   * Checks if a principal is authorized to perform an action on a resource. The action is specified
   * by an uri and an HTTP method.
   *
   * @param uri The uri of the action.
   * @param method The HTTP method of the action.
   * @param resourceId The involved resource.
   * @return The AccessControlResponse that contains the flag to determine if the action is
   *     authorized.
   */
  @SuppressWarnings("SpringOmittedPathVariableParameterInspection")
  @GetMapping("${avenirs.access.control.authorize}")
  public ResponseEntity<AccessControlResponse> isAuthorized(
    ...
    AccessControlResponse response =
        new AccessControlResponse()
            .setLogin(login)
            .setResourceId(resourceId)
            .setUri(uri)
            .setMethod(method);
    if (StringUtils.hasLength(uri) && StringUtils.hasLength(method)) {
      RBACActionRoute actionRoute =
          actionRouteService
              .getAllActionRoutesBySpecification(
                  RBACActionRouteSpecificationHelper.filterByURIAndMethod(
                      uri, method.toLowerCase()))
              .orElse(null);
  ...
  public ResponseEntity<AccessControlGrantResponse> grantAccess(
    ...
    try {
      ...
      response =
          new AccessControlGrantResponse()
              .setLogin(login)
              .setGranted(false)
  ...
  public ResponseEntity<AccessControlRevokeResponse> revokeAccess(
    ...
    try {
      ...
      response =
          new AccessControlRevokeResponse()
              .setLogin(login)
              .setRevoked(false)
              .setError(exception.getMessage());
    }
