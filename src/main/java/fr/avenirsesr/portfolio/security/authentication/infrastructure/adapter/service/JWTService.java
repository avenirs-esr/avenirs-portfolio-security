...
import fr.avenirsesr.portfolio.security.authentication.infrastructure.adapter.model.ModulusAndExponent;
...
public class JWTService implements JWTServicePort {
  ...
  private final JWTToCryptographicKeyAlgoMapper algoMapper;

  public JWTService(JWTToCryptographicKeyAlgoMapper algoMapper) {
  ...
  private Optional<ModulusAndExponent> fetchModulusAndExponent(String kid) {
    try {
      ...
      log.error(
          "fetchModulusAndExponent, unable to fetch Modulus and Exponent. HTTP Status code: {},"
              + " response body: {}",
   ...
   */
  protected Optional<ModulusAndExponent> extractModulusAndExponent(
      String jwksResponse, String kid) {
    try {
   ...
   */
  protected Optional<PublicKey> generateRSAPublicKey(
      String alg, ModulusAndExponent modulusAndExponent) {

    try {
      // Builds the key specification.
      RSAPublicKeySpec rsaPublicKeySpec =
          new RSAPublicKeySpec(modulusAndExponent.getModulus(), modulusAndExponent.getExponent());

      final Optional<String> keyAlgo = algoMapper.mapJWTToCryptographicKey(alg);
      if (keyAlgo.isEmpty()) {
        log.error(
            "generatePublicKey, unable to map jwt alg {}, from mapping defined in properties", alg);
        return Optional.empty();
      }
  ...
  /**
   * Gives the public key associated to an idToken header. If the key does not exist in the key
   * repository it is generated.
   *
      ...
      try {
        ...
        if (!this.keysRepository.containsKey(kid)) {
          ...
          if (publicKey.isEmpty()) {
            log.error(
                "getPublicKey, Error while trying to generate public key from kid{}, idToken: {}",
                kid,
                idToken);
            return Optional.empty();
          }
   ...
   *
   * @param accessToken The access token which contains the access token value and raw id token.
   * @return An Optional of claims as a map.
   */
  public Optional<Map<String, Object>> parseAndCheckSignature(OIDCAccessToken accessToken) {
    if (accessToken == null) {
      return Optional.empty();
    }

    OIDCIdToken idToken =
        accessToken.rawIdToken() != null ? new OIDCIdToken(accessToken.rawIdToken()) : null;
    final Optional<PublicKey> publicKey = getPublicKey(idToken);
    if (publicKey.isEmpty()) {
      log.error("Empty public key while trying to fetch claims from accessToken: {}", accessToken);
      return Optional.empty();
    }
    ...
    try {
      ...
      final Claims claims =
          Jwts.parser()
              ...
              .build()
              .parseSignedClaims(accessToken.accessToken())
              .getPayload();
      // Expiration check
      ...
      if (issuedAt > currentTime + iatTolerance) {
        log.warn(
            "Field iat of jwt is in the future iat: {}, currentTime: {}, iatTolerance: {}",
            issuedAt,
            currentTime,
            iatTolerance);
        return Optional.empty();
      ...
      log.error("Invalid signature of access token in accessToken: {}", accessToken);
      return Optional.empty();
    }
  }
}
